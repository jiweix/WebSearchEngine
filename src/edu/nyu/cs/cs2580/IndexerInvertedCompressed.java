...
public class IndexerInvertedCompressed extends Indexer implements Serializable {
    ...
    // Term frequency, key is the integer representation of the term and value is the number of times the term appears in the corpus.
    ...
    // private Map<Integer, int[]> _postingLists = new HashMap<>();
    ...
    public void constructIndex() throws IOException {
        ...
        // ---------- testing ---------------
//        generateTestingResultsFile();
    }

    // this is used to generate test results in a file for testing
    private void generateTestingResultsFile() {
        System.out.println("Generating testing data");
        // generate a list of products info
        StringBuffer results = new StringBuffer();
        for (DocumentIndexed doc : _documents) {
            results.append(doc.getProduct().toString() + "\n");
        }
        FileGen.generate(results.toString(), "data", "product-list");

        // TODO: too slow now, generate a list of product names after combined
//        LinkedList<Product> uniqueProductSet = new LinkedList<>();
//        uniqueProductSet.addAll(BestbuyFileProcessor.getUniqueProductSet());
//        LinkedList<Product> combinedUniqueProductSet = new LinkedList<>();
//        results = new StringBuffer();
//        while(!uniqueProductSet.isEmpty()) {
//            Product curr = uniqueProductSet.remove();
//            combinedUniqueProductSet.add(curr);
//            results.append(curr.getName() + "\n");
//            Vector<String> s1 = new Vector<>(Arrays.asList(curr.getName().split("[^\\w]+")));
//            Iterator<Product> iter = uniqueProductSet.iterator();
//            while (iter.hasNext()) {
//                Vector<String> s2 = new Vector<>(Arrays.asList(iter.next().getName().split("[^\\w]+")));
//                if (editDistance(s1, s2) < 3) {
//                    combinedUniqueProductSet.add(iter.next());
//                    results.append(iter.next().getName() + "\n");
//                    iter.remove();
//                    System.out.println("" + uniqueProductSet.size());
//                }
//            }
//        }

//        FileGen.generate(results.toString(), "data", "combined-product-list");
    }
    ...
    private void processDocument(int docid, String content) {
        ...
        // parse content
        Scanner s = new Scanner(content).useDelimiter("[^\\w]+");  // remove punctuation & whitespace
    ...
    public int documentTermFrequency(String term, int docid) {
        ...
        while (jmpPtr < docOccrArr.length) {
            if (docOccrArr[jmpPtr] == docid) {
                //System.out.println("term: "+term+" docid: "+docid+" freq: "+ docOccrArr[jmpPtr+1]);
    ...
    private int[] nextPhrase(String phrase, int docid, int pos) {
        ...
        if (isphrase) {
//      System.out.println("find phrase " + phrase + ", at " + docid);
    ...
    private int[] getPostingList(int token) {
        if (_tempPostingList.containsKey(token)) {
            ...
            if (_tempPostingList.keySet().size() > _tempPostingListSize) {
                // System.out.println("Number of Tokens: " + _tempPostingListSize);
